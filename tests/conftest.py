...
YAML_FILES: t.List[str] = sorted([x.name for x in WORKFLOWS_DIR.iterdir() if x.is_file() and x.suffix in {'.yml', '.yaml'}])
    ...
    if 'docker' in workflow_file_name:
        if 'pol0' in workflow_file_name:
            jobs = dict(jobs, **{
                ...
                for k, v in yaml_workflow['jobs'].items() if k.startswith('call_docker_job_after_ci')
            })
            assert len(jobs) == 8 if 'green' in workflow_file_name else 4, f"Failed to find all jobs in {workflow_file_name}"
        elif 'pol1' in workflow_file_name:
            jobs = dict(jobs, **{
                v.get('name', k): 'success'  # FORCE YES Docker
                for k, v in yaml_workflow['jobs'].items() if k.startswith('call_docker_job_after_ci')
            })
            assert len(jobs) == 7 if 'green' in workflow_file_name else 4, f"Failed to find all jobs in {workflow_file_name}"
            ...
            if 'green' in workflow_file_name:
                jobs = dict(jobs, **{
                    ...
                    yaml_workflow['jobs']['call_docker_job_after_ci_skip'].get('name', 'call_docker_job_after_ci_skip'): 'skipped',  # skip docker if ci tests skipped
                    yaml_workflow['jobs']['call_docker_job_after_pass_ci'].get('name', 'call_docker_job_after_pass_ci'): 'success',  # run docker if ci tests passed
                })
                assert len(jobs) == 7, f"Failed to find all jobs in {workflow_file_name}"
            else:
                jobs = dict(jobs, **{
                    # expected docker job conclusions, for policy 2 (aka CI/CD)
                    yaml_workflow['jobs']['call_docker_job_after_ci_fail'].get('name', 'call_docker_job_after_ci_fail'): 'skipped',  # skip docker if ci tests failed
                })
                assert len(jobs) == 4, f"Failed to find all jobs in {workflow_file_name}. Jobs: {jobs}"
        elif 'pol3' in workflow_file_name:  # CI/CDelivery
            if 'green' in workflow_file_name:
                jobs = dict(jobs, **{
                    # expected docker job conclusions, for policy 3 (aka CI/CDelivery)
                    yaml_workflow['jobs']['call_docker_job_after_ci_skip'].get('name', 'call_docker_job_after_ci_skip'): 'success',  # run docker if ci tests skipped
                    yaml_workflow['jobs']['call_docker_job_after_pass_ci'].get('name', 'call_docker_job_after_pass_ci'): 'success',  # run docker if ci tests passed
                })
                assert len(jobs) == 7, f"Failed to find all jobs in {workflow_file_name}"
            else:
    ...
    if 'pypi' in workflow_file_name:
        if 'red' in workflow_file_name:
            jobs = dict(jobs, **{
                ...
                for k, v in yaml_workflow['jobs'].items() if k.startswith('call_pypi')
            })
        else:
            jobs = dict(jobs, **{
...
def trigger_workflow():
    ...
    import requests
    def _trigger_workflow(workflow_resource: t.Dict[str, t.Union[str, int]], github_token: str) -> requests.Response:
...
def get_workflow_runs():
    ...
    import requests
    def _get_workflow_runs(workflow_resource: t.Dict[str, t.Union[str, int]], github_token: str) -> requests.Response:
...
def get_workflow_run_status():
    import requests
    def _get_workflow_run_status(workflow_run_resource: t.Dict[str, t.Union[str, int]], github_token: str):
        response = requests.get(
