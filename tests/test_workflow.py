...
def test_workflow_and_its_jobs_are_green(
    ...
    assert workflow_runs_response.status_code == 200, f"Failed to get workflow runs for Workflow {workflow_id}"
    ...
    while not runner_triggered:
        ...
        assert workflow_runs_response.status_code == 200, f"Failed to get workflow runs for Workflow {workflow_id}"
        workflow_runs: t.Dict = workflow_runs_response.json()
        new_latest_run: int = max(workflow_runs["workflow_runs"], key=lambda x: x["run_number"])
    ...
    workflow_finished: bool = (workflow_run := get_workflow_run_status({'id': run_id}, token).json())["status"] not in NOT_FINISHED_STATES
    while not workflow_finished:
        print(f"\n[DEBUG]: ----- Workflow Status: " + workflow_run["status"])
        time.sleep(5)  # Adjust the interval as needed
        workflow_finished = (workflow_run := get_workflow_run_status({'id': run_id}, token).json())["status"] not in NOT_FINISHED_STATES

    # Assertions on overall workflow status
    runtime_status: str = workflow_run["status"]
    expected_status: str = 'completed'
    assert runtime_status == expected_status, f"Workflow '{workflow_name}' ID {workflow_id} and RUN ID {run_id} Completed with status: {runtime_status}. But We expected status {expected_status}."

    runtime_conclusion = workflow_run["conclusion"]
    expected_conclusion = expectations["conclusion"]
    assert runtime_conclusion == expected_conclusion, f"Workflow '{workflow_name}' ID {workflow_id} and RUN ID {run_id} Completed with conclusion: {runtime_conclusion}. But We expected conclusion {expected_conclusion}."
    ...
    assert jobs_info_response.status_code == 200, f"Failed to get jobs info for {workflow_run['run_number']}. Reported run_number's: {[i['run_number'] for i in workflow_runs['workflow_runs']]}"
    ...
    for job in jobs_info["jobs"]:
        ...
        assert job["conclusion"] == expected_job_conclusion, f"Job {job['name']} Completed with conclusion: {job['conclusion']}. But We expected conclusion {expected_job_conclusion}."
